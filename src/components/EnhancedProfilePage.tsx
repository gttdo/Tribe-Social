...
import { supabase, checkAuthStatus, getCurrentSession } from '../utils/supabase/client';
import { UserResult, UserInfo } from '../App';
import { FeedPost, FeedComment } from '../utils/social-feed-types';
import { transformBackendPost } from '../utils/social-feed-helpers';
...
// Enhanced achievements with XP progression context
const getAchievements = (userStats?: UserProfileData): Achievement[] => [
  {
    id: '1',
    name: 'Welcome Warrior',
    description: 'Joined the Tribe community and completed your profile',
    icon: Sparkles,
    color: 'neon-lilac',
    earned: true,
  ...
  },
  {
    id: '4',
    name: 'Rising Star',
    description: 'Received 50 likes across all your posts',
    icon: TrendingUp,
    color: 'glitch-red',
    earned: false,
    xpReward: 300,
  ...
  },
  {
    ...
    description: 'Reached level 5 in your Tribe journey',
    icon: Trophy,
    color: 'electric-blue',
...
export function EnhancedProfilePage({ userResult, userInfo, onBack, onLogout, onNavigateToPage, isOwnProfile }: EnhancedProfilePageProps) {
  ...
  // Get theme colors based on selected theme or default to neon-lilac theme
  ...
  // Create fallback profile data from userResult and userInfo
  ...
  // Handle post deletion with refresh
  ...
  // Handle post click for editing/deletion options
  const handlePostClick = async (post: UserPost | any) => {
    try {
  ...
  const checkAuth = async () => {
    try {
      ...
      if (!session?.user) {
        console.log('No authenticated user - using fallback profile data');
      ...
      console.error('Auth check failed in ProfilePage:', error);
      setProfileData(createFallbackProfileData());
      setError(null);
      setAuthChecked(true);
      return null;
  ...
  // Fetch user posts using Edge API
  const fetchUserPosts = async (userId: string) => {
    try {
      console.log('=== FETCHING USER POSTS VIA EDGE API ===');
      
      if (!userId) {
        console.warn('Profile Posts: missing userId; skip fetch to avoid 400s.');
        setUserPosts([]);
        return;
      }

      console.log('Resolved userId for posts fetch:', userId);
      ...
      if (data && data.posts && Array.isArray(data.posts)) {
        ...
        console.log(`✅ Loaded ${formattedPosts.length} posts via Edge API for userId: ${userId}`);
      ...
      console.log('Posts fetch info:', errorMessage);
      setUserPosts([]);
    }
  };
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (userId) {
        console.log('Authenticated user found, loading full profile data...');
  ...
  // Handle bio update with optimistic UI
  ...
  // Initial load
  useEffect(() => {
    console.log('=== ENHANCED PROFILE PAGE MOUNTED ===');
    loadData();
  }, []);

  // Get achievements with current user stats context
  ...
  const renderEmptyPostsState = () => (
    <div className="text-center py-12">
      ...
      <h3 className="text-lg font-medium text-pearl-white mb-2">No posts yet</h3>
      <p className="text-muted-lavender/70 mb-6">Share your first post to get started!</p>
      <Button
        onClick={() => onNavigateToPage?.('create')}
          ...
          <TabsContent value="posts" className="space-y-4 mt-6">
            {loading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {[...Array(6)].map((_, i) => (
                  <Skeleton key={i} className="h-48 bg-midnight-black/50" />
                ))}
              </div>
            ) : userPosts.length === 0 ? (
