    ...
    async (c) => {
      try {
        ...
        if (commentsError) {
          ...
          return c.json(
            { error: "Failed to fetch comments", details: commentsError.message, code: commentsError.code },
            500,
          );
        }
...
export function setupRoutes(app: Hono) {
  ...
  app.get("/make-server-70df0d6e/posts", async (c) => {
    try {
      ...
      const transformedPosts = (postsData || []).map((post) => ({
        ...
        nickname: post.author?.username || "Unknown User",
        profileImageUrl: post.author?.avatar_url,
  ...
  app.post("/make-server-70df0d6e/posts", async (c) => {
    try {
      ...
      if (insertError) {
        ...
        return c.json({ error: "Failed to create post", details: insertError.message, code: insertError.code, hint: insertError.hint }, 500);
      }
  ...
  app.get("/make-server-70df0d6e/users/profile", async (c) => {
    try {
      ...
      const { data: userData, error: profileFetchError } = await supabaseAdmin
        .from("profile")
        .select(`
          id, username, email, phone,
      ...
      if (profileFetchError) {
        console.error("Error fetching user data:", profileFetchError);
  ...
  app.patch("/make-server-70df0d6e/users/profile", async (c) => {
    try {
      ...
      // Execute updates on the profile table
      // Include bio update in the same update object
      if (bioUpdate !== null) {
  ...
  app.get("/make-server-70df0d6e/posts/:postId", async (c) => {
    try {
      ...
      const { data: userData, error: userError } =
        await supabaseAdmin
          .from("profile")
          .select("username, avatar_url, core_realm")
  ...
  const handleProfileUpdate = async (c) => {
    try {
      ...
      // Process description update (legacy support - maps to bio column)
      if (
        ...
        updates.bio = trimmedDescription;
        console.log(
          "Description update:",
      ...
      const transformedProfile = {
        ...
        profileImageUrl: updatedProfile.avatar_url,
